predic_svm_senti<-function(texto){
  ...
  nueva_copur_limpio<-limpiar_corpus_senti(nueva_corpus)
  #Transformar corpus a DTM
  obs_dtm<-DocumentTermMatrix(nueva_copur_limpio)
  #obs_df <- obs_dtm %>% as.matrix() %>% as.data.frame()
  obs_df <- as.data.frame(as.matrix(obs_dtm))
  test_nueva<-dplyr::bind_rows(test1,obs_df)
  test_nueva[is.na(test_nueva)] <- 0
  
  svm_pred_senti <- predict(svm_mod_senti,          
                            newdata = test_nueva)
  
  prediccion<-as.character(svm_pred_senti)
  return(prediccion)
}
